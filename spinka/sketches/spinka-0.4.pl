...
if ($#ARGV != 2) {die "Program used with parameters [list of variations] [SIN] [PIN]\n";}
...
for (my $j=0;$j<$#wang_sin+1;$j++){
	...
	for(my $i=0;$i<$#variat+1;$i++){
		if($variat[$i][1] eq $sin[$j][0]){
		  $node1{$inter}=$variat[$i][0]."_".$sin[$j][2];
		  $k++;
		  $din{$inter}++;
		  my $pos = $variat[$i][5];
		  my $ds =$sin[$j][3];
		  ...
		  $node2{$inter}=$variat[$i][0]."_".$sin[$j][5];	
		  $k++;
		  $din{$inter}++;
		  my $pos = $variat[$i][5];
		  my $ds =$sin[$j][6];
...
foreach my $klucz (keys %din){
	...
	if($dvar{$tab[0]} > 0){$GVinD{$tab[0]}=1;}#GV inside domain
	if($dvar{$tab[1]} > 0){$GVinD{$tab[1]}=1;}#GV inside domain
	if($cout{$tab[0]} > 0){$GVinD{$tab[0]}=0;}#GV outside domain
	if($cout{$tab[1]} > 0){$GVinD{$tab[1]}=0;}#GV outside domain
