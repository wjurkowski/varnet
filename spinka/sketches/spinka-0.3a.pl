...
for(my $i=0;$i<$#variations+1;$i++){
	my @tab = split(/\t/,$variations[$i]);#Gene_symbol, Entrez, Ref AA, Position, Var AA
...
foreach my $klucz (keys %subsy){
	...
	if(($dvar{$tab[0]} > 0) and ($dvar{$tab[1]} > 0)){
		printf OUT3 "$klucz\n";#print DIN subnetwork with SNV in domain
	}	
	if(($cout{$tab[0]} > 0) and ($cout{$tab[1]} > 0)){
		printf OUT4 "$klucz\n";#print DIN subnetwork with SNV outside the domain
	}	
