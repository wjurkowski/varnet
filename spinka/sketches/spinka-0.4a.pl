...
if ($#ARGV != 3) {die "Program used with parameters [list of variations] [SIN] [PIN] [HGNC mapping]\n";}
...
my $wangi=$DB."/From_WangX/".$ARGV[1];
my @wang_sin=open_file($wangi);
my (@sin);	
...
my $wangi2=$DB."/From_WangX/".$ARGV[2];
my @wang_pin=open_file($wangi2);
my (@pin);
...
#parse HGNC mapping of Entrez Gene ID on HGNC gene symbols 
my $hgncfile=$DB."/mapping_data/".$ARGV[3];
my @hgncmap=open_file($hgncfile);
my (%hgnc);
for(my $i=0;$i<$#hgncmap+1;$i++){#parse interactions
	my @tab=split(/\t/,$hgncmap[$i]);
	$hgnc{$tab[4]}=$tab[1] if exists $tab[4];
#print "kiki $tab[4]\n";
}
...
for (my $j=0;$j<$#wang_sin+1;$j++){
	...
	for(my $i=0;$i<$#variat+1;$i++){
		if(($variat[$i][1] eq $sin[$j][0]) or ($variat[$i][1] eq $sin[$j][1])){
		  $node1{$inter}=$hgnc{$sin[$j][0]}."_".$sin[$j][2];
		  $node2{$inter}=$hgnc{$sin[$j][1]}."_".$sin[$j][5];	
		  $din{$inter}++;
		}
...
for (my $j=0;$j<$#wang_pin+1;$j++){
	for(my $i=0;$i<$#variat+1;$i++){
		my $intEntrez = $pin[$j][0]."\t".$pin[$j][1];#edge defined by pair of Entrez Gene ID
		if($variat[$i][1] eq $pin[$j][0]){$edges{$intEntrez}++;}
		elsif($variat[$i][1] eq $pin[$j][1]){$edges{$intEntrez}++;}
	}	
}		
